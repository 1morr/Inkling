...
`NoteItemSlots`，三條規則:內容沒變的筆記沿用自己上一輪的 `ListItem`**而且一個屬性都不碰**、
消失的那一則把物件讓給後繼者、**內容變了的則給一列全新的**。
配套是 `RaiseItemsChanged(CmdPalRefresh.KeepSelection)` —— **兩件事缺一不可**,
只做一件完全沒有效果。

第三條反直覺但**不能拿掉**:就地改一個 CmdPal 已經建好 view model 的清單項
(`Command` / `MoreCommands` / `Details` 每一個都算),CmdPal 會立刻把那一列渲染出來 ——
而「內容變了」最常見的來源是使用者**正在編輯那一則**，他不在清單頁上。
症狀是編輯表單旁邊多出一塊預覽、底部工具列變成清單那一列的命令。
讓槽那條路仍然是就地更新，所以 `ApplyNote` 那類方法**每一項都要設一遍**
(命令、標題、副標、圖示、詳細窗格、選單、標籤)，漏一項就是「顯示甲、命令綁著乙」
而且完全靜默。加新的清單頁時記得一起走這條路，見第 13 條與
...
8. **回饋一律走 `Feedback`，通道由面板去留決定。三個方法，沒有第四種。**
   ...
   **判準是「使用者接下來還要不要看著這個面板」。** 不要的話走 `Done` ——
   新增與編輯表單的存檔、快速記下、記下並預覽頁的「完成」、隨手草稿的存檔都是，而它們跟關掉
...
13. **選取跟著 `IListItem` 的參考走，而 `RaiseItemsChanged()` 的預設參數會把它推回第一列。**
    ...
    **但內容變了的那一列要給全新物件**:就地更新會被 CmdPal 渲染到使用者當下看的
    那一頁上(編輯存檔時最明顯)，那是 `NoteItemSlots` 的第三條規則。
...
- <a id="copy-source"></a>**對外文案只有一個來源:[`docs/copy.md`](docs/copy.md)。**
  Store listing 的三份文字**不在 repo 裡**(在 Partner Center)，先改 Store 再回頭補的話，
  下一個人打開 repo 看到的是舊的，漂移是必然的。改文案的順序永遠是:先改 `docs/copy.md`，
  再同步到 Store 的三個語言、`docs/gallery/extension.json`、`Package.appxmanifest` 的
  兩處 `Description`、兩份 README 的頂部那一句。**Store 端要開一個新的 submission 才生效**
  (套件沿用上一次的，不必重傳)，見 [`docs/release-runbook.md` 第 17 步](docs/release-runbook.md)。
  那一份的開頭釘著三條規則，寫新文案之前先讀:
  - **alias 是可選的加速方式，不是第一步。** 預設路徑「叫出面板 → 打 Inkling → 選快速記下」
    不設定任何東西就走得完;alias 與全域快速鍵一律放後面，用「想更快的話」的語氣帶出來。
    2026-08-25 之前每一處文案都把它寫成必做步驟(Store 描述的第一句就是「設一個 alias」)，
    讀起來像「不設就不能用」。
  - **講做什麼，不講為什麼。** 對外只寫按哪個鍵、發生什麼事、檔案存到哪;考證留在
    `design-notes.md`，README 用連結指過去。破折號堆疊、警句式的因果句、三段排比一律拆掉 ——
    那是這個 repo 的文案最容易漂回去的三個毛病。
  - **實作語彙不要進 Store。** `Note.IsExternal`、front matter 的 id 怎麼設計、
    「沒有宣告網路能力」這種話使用者看不懂;隱私講一句「筆記只留在你自己的電腦上，
    Inkling 不蒐集也不傳送任何資料」就夠了。
- <a id="chinese-comma"></a>**中文的逗號用全形「，」。** 2026-08-25 一次掃過全樹(6082 處)，
  範圍是所有中文文字:三份 `.resx` 的介面字串、兩份 README、`docs/*`、這一份、`CHANGELOG.md`、
  `.claude/skills/*`、workflow 與腳本的註釋、以及所有程式碼註釋。
  **其餘標點維持半形**(括號、冒號、分號)—— 它們在這個 repo 裡幾乎都夾著程式碼、路徑或英文，
  換成全形只會讓行寬跳動。判準是「逗號有沒有貼著漢字」，所以英文句子與程式碼裡的逗號不動。
- <a id="docs-language"></a>**文檔語言分層。** 判準只有一條:**讀者是不是維護者以外的人。**
  ...
  - **維護者文檔只有繁體中文，不翻** —— 這一份、`docs/*.md`、`CHANGELOG.md`、
    `.claude/skills/*`、以及所有程式碼註釋。`docs/copy.md` 也算在這裡:**說明是繁中，
    裡面裝的成品文字是三個語言**，那些字要一字不差地貼到 Store 與 gallery 去。
...
- 改了指令、設定項、資料格式或對外行為，同一輪更新**兩份 README**、
  ...
  改一份就改另一份。**對外文案(Store、gallery、manifest 的描述、README 頂部那一句)
  不是從 README 抄出去的**，來源是 [`docs/copy.md`](docs/copy.md)，見上面那一條。
  **文檔各有各的家，別放錯:**
