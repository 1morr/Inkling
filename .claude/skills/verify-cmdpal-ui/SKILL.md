...
**`show` 不保證停在主搜尋框。** 面板已經被焦點轉移隱藏的時候,`show` 開回**主頁**;
但面板**還開著**的時候按熱鍵,它停在原來那一頁 —— 於是 `show|type:...|key:Enter`
會把字打進上一次留下的那一頁。實測踩過:上一串停在快速記下頁,下一串的
`type:Inkling|key:Enter` 就靜靜地多存了一則叫「Inkling」的筆記。
序列開頭多送幾個 `esc`(面板不在前景時會自動跳過,不花成本),
或者第一個 `tree` 先確認 placeholder 是主搜尋框的那一句。
