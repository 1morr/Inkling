...
編輯頁在單行標題欄按 `Ctrl+Enter` 會開外部編輯器並收掉面板，那條路的訊息會說「不在前景」、
...
**序列的預期結果本來就是「面板收起來」的話，一定要帶 `-Retries 1`。** 跳外部編輯器
(編輯頁 / 隨手草稿的 `Ctrl+O`)、表單存檔、記下並預覽頁的「完成」都是這種:面板收掉之後
後面的步驟必然判定不可用，預設值會讓整串跑滿四次 —— 實測就這樣把同一個檔案在 VS Code 裡
開了四次(2026-08-21，那次意外也是「位置鍵打不打得到工具列」唯一的原始證據)。

**⚠ 表單頁上的位置鍵不一定送得進去。** `Enter` 與 `Ctrl+Enter` 按的是底部工具列，
但在 Adaptive Cards 的**多行**輸入框裡它們會被輸入框吃掉，**單行**框裡才會冒泡上去;
`RequestedShortcut`(例如 `Ctrl+O`)則一律送得到 —— CmdPal 在 tunneling 階段就攔下來了。
所以「按了沒反應」在表單頁上常常不是 bug，是焦點在多行框裡。量測表見
`docs/design-notes.md` 的〈位置鍵打不打得到工具列〉。
