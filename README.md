...
- **空的 `tags` 不寫。** 有值時照樣寫成 `tags: [a, b]`,別的編輯器加的標籤也原樣留著,
  但沒有值就整行省略。理由是 front matter 是**手機上最先看到的東西** ——
  OneDrive / Google Drive 的手機 App 不渲染 Markdown,那幾行就是純文字擋在最上面,
  而 `tags` 目前還沒有任何功能,不值得佔那一行。既有檔案裡的 `tags: []` 照樣讀得懂,
  不需要遷移;經過 Notelet 編輯一輪之後那一行會自己消失。
...
- **對話框不掛 owner,所以它在工作列上有自己的按鈕。** 這是刻意的:那顆按鈕是「對話框
  ...
  (代價:那顆按鈕的圖示是套件的 `Square44x44Logo`,目前還是 Visual Studio 模板留下的
  空白方框 —— 那是圖示還沒做,不是對話框的問題。)
