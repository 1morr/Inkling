...
| 隨手草稿 | `Inkling：隨手草稿` 開一塊永久的便條紙,打開就是上次留下的東西,不必取標題。**沒有自動儲存**(CmdPal 做不到,[為什麼](docs/design-notes.md#scratchpad-no-autosave)):`Tab` → `Enter` 存檔,**存完面板自己收起來**;`Ctrl+O` 跳到系統預設編輯器,自動儲存在那邊 |
...
編輯,不是檔案裡的草稿)。**存檔是 `Tab` 到「儲存」再按 `Enter`,存完面板自己收起來。**
