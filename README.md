...
| 新增(完整) | `Inkling：新增筆記` 開表單,可寫多行內文;在瀏覽筆記的清單頁按 `Ctrl+N` 也能直接開 |
| 瀏覽與搜索 | 標題與內文都能搜,多個關鍵字是 AND,標題命中排前面;副標是內文的第一行摘要。搜不到時會直說「找不到符合的筆記」(而不是「還沒有任何筆記」),按 Enter 直接進快速記下 |
| Markdown 預覽 | 選中筆記按 Enter 看渲染結果 |
| 原始文字 | `Ctrl+U` 在渲染結果與原文之間切換:清單頁換的是右邊的詳細窗格,預覽頁換的是整頁。**狀態全域共用而且記得住**;預覽頁的原文走純文字檢視器,縮排、連續空行、貼進去的 HTML / SVG 通通看得到(那些東西渲染完會整段消失) |
...
`Ctrl+U` 在「渲染結果」與「原文」之間切換,用途是看到檔案裡真正的字元:標題的 `#`、
粗體的 `**`、連結的 `[](…)` 渲染完就消失了,而**貼進來的 HTML / SVG 更是整段不見** ——
畫面看起來像空白,其實內容一直都在。

兩個畫面切的是不同的東西,但**狀態是同一個**:

| 在哪裡按 | 換掉什麼 | 原文長什麼樣 |
|---|---|---|
| 清單頁 | 右邊的詳細窗格(清單不重建、選中項不會跳掉) | 逐字逃脫的 Markdown;沒有外框與底色,但**行首縮排與連續空行會被渲染器正規化** |
| 預覽頁 / 記下並預覽頁 | 整頁的內容 | CmdPal 的純文字檢視器:等寬字、**一個字元都不動**。在內容上按右鍵還能切自動換行、等寬與縮放(那是 CmdPal 給的) |

狀態**全域共用而且記得住**:在預覽頁切掉,回到清單頁就是原文;關掉 CmdPal、重載擴展、
重開機都還在(存在 `settings.json` 的 `Inkling.ShowSource`)。設定頁上刻意沒有這一項 ——
切換鍵本身就是它的介面。刪除筆記那一頁的詳細窗格也跟著這個狀態走,但那一頁**沒有**切換鍵
(它的第一列是「刪除全部」,不值得為了切換多一條讓焦點跳過去的路)。

為什麼清單頁那邊不能就地改 `Details.Body`(那條通知路跨進程是斷的)、為什麼預覽頁用的是
`PlainTextContent` 而不是同一套逃脫,見[設計考證〈原始文字模式〉](docs/design-notes.md#source-mode)。
...
設定頁上只有三項。`settings.json` 裡還有第四個鍵 `Inkling.ShowSource`(原始文字模式),
那是 `Ctrl+U` 自己寫回去的檢視狀態,刻意不放進表單 —— 切換鍵就是它的介面。
快速記下沒有前綴設定 —— 它的入口(alias、全域快速鍵)由 CmdPal 那邊管,
