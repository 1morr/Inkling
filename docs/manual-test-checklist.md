...
- [ ] 記三則筆記,執行 `Notelet:刪除所有筆記`
- [ ] 確認框的標題寫著 **刪除全部 3 則筆記?**,數字與實際則數相符
      (數量是按下命令時才去讀的 —— `TopLevelCommands()` 不能碰磁碟)
- [ ] 這一個的**預設按鈕是「取消」**(跟單則刪除相反,刻意的)
- [ ] 取消 → 三則都還在
- [ ] 再執行一次並確認 → toast 說「已把 3 則筆記移到資源回收筒」,並回到主畫面
- [ ] 清單變成空的,顯示「還沒有任何筆記」
- [ ] 資源回收筒裡有那三個檔案,還原後三則筆記都回來
- [ ] **資料夾一則筆記都沒有時**執行它,toast 說「沒有筆記可以刪除」,不跳確認框
- [ ] 資料夾裡放一個非 `.md` 檔案(例如 `.txt`),執行刪除全部,**那個檔案不受影響**
