...
**焦點不會被搶**(表單後面那塊空白內容擋掉的,**曾經壞掉,不要略過**):

- [ ] 設定 → Extensions → Notelet 那一頁開著 → 切回 CmdPal 主視窗 → `Ctrl+K` → 設定 →
      改點東西按儲存 → 焦點**留在主視窗**,背景那個設定視窗沒有跳到前面來
      (壞掉時的症狀就是它跳到前面。兩個入口共用同一個頁面實例,存檔後我們一律叫 CmdPal
      重讀表單 = 控件重建 + 再一次 `Loaded`,而 `ContentFormControl` 載入後會自動聚焦。
      靠表單後面那塊空白內容讓 `OnlyControlOnPage` 變成 false 才擋得掉 ——
