...
- [ ] 有四個設定項,由上到下:筆記資料夾、快速記下的分隔符、記下後先看一眼、詳細面板寬度
...
- [ ] 詳細面板一開就是**寬**(1:1),設定頁的下拉選單顯示的也是「寬(1:1)」,不是空白
      (`ChoiceSetSetting` 是唯一沒有 defaultValue 建構子的設定型別,預設值靠物件初始化器補)
- [ ] 「記下後先看一眼」一開就是**勾著的**
- [ ] 筆記資料夾是 `%OneDrive%\Notelet`,分隔符是 `;;`
- [ ] 把 `Notelet.DetailsWidth` 手動改成一個不存在的檔位(例如 `"huge"`)再 Reload:
      寬度是**寬**,下拉選單顯示的也是「寬(1:1)」 —— 認不得的值跟預設同一個答案,
      而且設定頁顯示的跟實際生效的一致

**舊設定不會被更新洗掉**(改預設值那次的回歸驗證):

- [ ] 設定頁把寬度選成「窄」存檔 → 重新部署 + Reload → 還是**窄**
      (預設值只對「檔案裡沒有那個鍵」的情況生效)
- [ ] `settings.json` 裡 fallback 時代的孤兒鍵(`Notelet.QuickCapturePrefix` 之類)
