...
- [ ] 🤖 改完按「儲存」→ 底部命令列出現 **「已儲存：<標題>」的 InfoBadge**
      ...
      0.11.11762.0 上不動(對照組:新增表單的 `GoHome()` 正常),見
      [`known-issues.md` K-4](known-issues.md#k-4)。回上一頁請自己按 `Alt+Left` 或 `Esc`
...
- [ ] 🤖 編輯表單上按 `Ctrl+K` → 選單裡**只有「改用預設編輯器開啟」(`Ctrl+O`)這一項**,
      底部工具列的主命令也是它。多出「刪除」之類的列就是回歸 —— 在表單上刪掉正在編的
      東西沒有道理。⚠ 這一項會**收掉面板並丟掉卡片上還沒存的字**,見
      [`known-issues.md` K-6](known-issues.md#k-6)
...
- [ ] 🤖 分隔符的說明裡**看不到反引號**(Adaptive Cards 的 `TextBlock` 不認得它,
      會照字面印出來 —— 所以那段字用的是「」)
- [ ] 👀 ⚠ **這一項目前是失敗的,先當觀察項**:卡片有宣告 `"separator": true`
      (`InklingSettingsForm.cs:272,305`),但 2026-08-22 實測畫面上**沒有分隔線**
      (逐列掃描像素找不到任何整列非背景色)。哪天看到線了,把這一行改回一般驗證項
