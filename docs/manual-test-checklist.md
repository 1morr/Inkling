...
⚠ **四條路全部留在設定頁,一條都不導覽。** 這不是偏好而是先決條件:InfoBadge 綁在
當下這一頁上,回傳 `GoHome()` 會把訊息連同這一頁一起拆掉,**什麼都不會出現**
(2026-08-23 在這一頁上 A/B 過,成功那兩條當時就是這樣一個字都沒發出來)。
**驗到「有徽章但回到了主頁」就是回歸**,那個組合不成立。取樣點約 800 毫秒:

- [ ] 🤖 正常存檔 → **留在設定頁**,底部命令列出現 InfoBadge **「設定已儲存」**;
      按 `Esc` 才回上一頁
...
- [ ] 🤖 資料夾欄位填**不存在但完整**的路徑(例如 `C:\InklingTest\Sub`)→ **照樣存**,
      **表單留在原地**,InfoBadge 是
      **「已儲存 —— 這個資料夾還不存在,第一次存檔時會建立：<路徑>」**
      (打錯一個字就靜靜換了家,所以要當場講 —— **這一則是四條裡最不能丟的**);
      之後真的記一則,那個資料夾被建出來
