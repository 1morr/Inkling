...
- [ ] 🤖 按 `Ctrl+Shift+C` → **面板還開著、還停在清單頁,選中的還是同一列**
      (`…|key:Ctrl+Shift+C|wait:500|toast|tree:5`:`toast` 那兩行要看到
      `toast … 可見=True 前景=False` 配 `主面板 … 前景=True`,而 `tree` 還在清單頁)
      ⚠ **面板消失就是回歸**,而且成因幾乎一定是漏掉 `ToastArgs.Result = KeepOpen()`
      —— 預設是 `Dismiss`
- [ ] 🤖 跳出一則 toast,內容是 **`已複製:<那一則的標題>`**(標題要對,不是別列的)
- [ ] 👀 toast 畫在**面板下方、不蓋住面板**(量到:面板底邊 y=1404、toast 頂邊 y=2005)
...
- [ ] 🤖 **空內文不會清掉剪貼簿**:先複製一段別的文字,再選中一則**沒有內文**的筆記按
      `Ctrl+Shift+C` → toast 寫 **`沒有內文可以複製`**(**不帶標題** —— 什麼都沒進剪貼簿),
      而且**剛剛複製的東西還貼得出來**
...
- [ ] 🤖 預覽頁按 `Ctrl+Shift+C` → **面板還開著、還停在預覽頁**,貼得出內文,
      而且**跳同一則 `已複製:<標題>` toast**。⚠ 這一頁以前是**完全靜默**的
      (理由「使用者正看著剛複製走的內容」站不住:按下去畫面一個像素都不變),
      沒有 toast 就是回歸
- [ ] 🤖 **標題改過之後 toast 要跟著改**:在預覽頁 `Ctrl+E` 改標題存檔,回到預覽頁按
      `Ctrl+Shift+C` → toast 講的是**新標題**。漏掉 `NoteTitle` 的同步就會講舊的
- [ ] 🤖 `Ctrl+E` 進編輯表單
...
⚠ **通道跟面板去留是兩件事,別再綁在一起想。** 收面板的是 `ToastArgs.Result`
(預設 `Dismiss`,所以 `ShowToast("字串")` 那個簡寫會關),不是 toast 本身 ——
toast 拿不到前景,見[設計考證〈toast 不會把面板關掉〉](design-notes.md#toast-does-not-steal-focus)。
**這一節所有失敗路徑的 `Result` 都是 `KeepOpen`**:失敗當下把面板收走,等於把證據一起收走。
