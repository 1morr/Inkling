...
上擠掉了 —— 那個位置的規則見〈`Ctrl+Enter` 三頁一律是「編輯」〉。
...
兩個鍵能觸發(預覽頁的編輯:`Ctrl+E` 與 `Ctrl+Enter`),而這也是為什麼底部按鈕上寫的是
`Ctrl+⏎` 而不是那個命令自己的鍵。
...
原本三頁各自「最自然的兩步」:清單頁 `Enter` 預覽 / `Ctrl+Enter` 編輯,預覽頁 `Enter` 編輯 /
`Ctrl+Enter` 順位落在複製內文,記下並預覽頁 `Enter` 完成 / `Ctrl+Enter` 編輯。單看每一頁都
說得通,合起來就是「`Ctrl+Enter` 在這一頁是編輯、在那一頁是複製」,得記兩套。

現在**三頁一律 `Ctrl+Enter` = 編輯**,做法是讓兩個 `ContentPage` 的第一項都是「完成」
(收起面板)、第二項都是「編輯」。付出的代價寫清楚:

- **預覽頁的 `Enter` 從「編輯」變成「完成」** —— 誤按會把面板收掉,得重新叫出來再搜一次。
  換來的是「看完了就 `Enter` 收工」跟記下並預覽頁同形,而編輯還有 `Ctrl+E` 與 `Ctrl+Enter`
  兩條路。
- **複製內文只剩 `Ctrl+Shift+C`** —— 它本來就是那個鍵,只是預覽頁上曾經順帶多一個
  `Ctrl+Enter`。

`Inkling：刪除筆記` 那一頁**刻意不跟**:它的 `Enter` 是「刪除,先問一次」、`Ctrl+Enter` 是
