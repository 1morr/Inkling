...
順帶一個踩到的細節:**預覽頁的命令陣列裡,第二個掛的是 `Ctrl+Enter`**(`ContentPage`
的第一個是底部工具列主按鈕),跟清單頁「第一個 `MoreCommands` 才是 `Ctrl+Enter`」不一樣。
切換原始文字一開始排在第二個,結果把「複製內文」從 `Ctrl+Enter` 上擠掉了(真機驗證時
底部工具列直接看得出來),所以它排在複製後面。
