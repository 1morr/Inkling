...
## K-18 `GoHome()` 把 InfoBadge 一起吃掉,用完整表單新增筆記完全沒有確認

**嚴重度:應修。** 2026-08-23 全量驗證時在真機上發現。

**重現**

1. 執行 `Inkling：新增筆記`,填標題與內文,按「儲存」。
2. 面板跳回首頁。

**預期**(`manual-test-checklist.md` §7 就是這樣寫的):底部出現「已新增：<標題>」的 InfoBadge。

**實際**:首頁上**沒有任何 InfoBar 或 InfoBadge**。按下 Enter 之後
400 / 900 / 1500 / 2500 ms 四次截圖都沒有,UIA 樹裡也搜不到「已新增」。

**證據鏈**

- 檔案確實建立了,所以 `NoteFormContent.cs:161` 的
  `new ToastStatusMessage(Strings.Format(Resources.NoteCreated, title)).Show()`
  **一定執行過**;接著 `return AfterSave`,而新增路徑的 `AfterSave` 是 `GoHome()`。
- **對照組**:同一段程式的編輯分支(`:166`)發同樣形狀的訊息,但回的是 `KeepOpen()`,
  「已儲存：<標題>」的 InfoBar + InfoBadge **看得見**。存檔失敗那條(`:180`,也是 `KeepOpen`)
  同樣看得見。
- 也就是說訊息發得出去,是**導覽把它拆掉了** —— CmdPal 的 status InfoBar 綁在當下那一頁的
  view model 上。

**影響**:走完整表單新增筆記的人得不到任何成功回饋,只看到面板跳走。分不出「存好了」與
「什麼都沒發生」,有可能再存一次。快速記下那條不受影響(它走的是
`CommandResult.ShowToast`,那是 CmdPal 自己畫的另一個視窗)。

**建議修法**(擇一,動手前先確認 CmdPal 是否真的在導覽時清掉 InfoBar):

1. 新增路徑改回 `KeepOpen()`,靠 InfoBadge 說話 —— 但那樣就跟編輯表單一樣停在原地,
   而新增完的下一步通常是收工,體感會變差。
2. 改用 `CommandResult.ShowToast`。**這其實符合硬規則 8 的判準** ——
   判準不是「這裡會不會關面板」,而是「使用者接下來還要不要看著這個面板」,
   新增完就是收工。快速記下那條走的就是這個。
3. 先 `Show()` 再延遲導覽 —— 最脆弱,不建議。

---

## 2026-08-22 那一輪的 17 條去哪了
