...
## K-20:刪除頁對「讀不出來的檔案」一個字都不提

**嚴重度:建議。** 2026-08-23 v1.0.0 送審前的全量驗證抓到的。

清單頁在掃描時會把讀不出來的 `.md`(被別的程式獨佔開著、或編碼不是 UTF-8)計進
`SkippedFileCount`,並在清單最後多一列「有 N 個檔案讀不出來 / 檔案還在資料夾裡 ——
可能被別的程式開著,或者編碼不是 UTF-8」。**刪除頁沒有這一列**,於是同一個資料夾
在兩頁上的說法不一致 —— 尤其是資料夾裡「只剩」讀不出來的檔案時,刪除頁顯示
「沒有筆記可以刪除」,而檔案總管裡明明有 `.md`。

重現(實測過的那一條):

1. 把筆記資料夾指到測試資料夾,放兩則筆記。
2. 另一個 PowerShell 視窗:
   `$fs = [IO.File]::Open("<資料夾>\某則.md", 'Open', 'ReadWrite', 'None')`
3. 再動一個檔案讓 watcher 觸發重掃(repository 有記憶體快取,單純鎖檔不會重掃)。
4. 清單頁:多一列「有 1 個檔案讀不出來」。**刪除頁:只有「沒有筆記可以刪除」。**

建議修法:`DeleteNotesPage` 沿用 `NoteListPage` 那一列的做法,把
`SkippedFileCount` 也接出來 —— 兩頁共用同一組資源字串,不要各寫一份
(「內文的第一行有效文字」漂移過,同樣的原因)。**那一列要是 command-less 的資訊列**,
別讓它變成一個按下去會刪東西的目標。

為什麼沒有擋著發版:觸發條件是「檔案正被別的程式獨佔開著、或編碼壞掉」,
而那兩種情況清單頁都講得出來,使用者的主要入口也是清單頁;刪除頁少講一句話
不會造成資料損失,也不會讓人以為筆記被刪掉了(檔案還在磁碟上)。

---

下面那一節是修掉之後的去向索引,不是待辦清單。
