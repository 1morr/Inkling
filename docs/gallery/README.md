...
- `extension.json` — 投稿用中繼資料草稿。送出去之前要確認:
  ...
  - `homepage`:目前寫的是推測的 `https://github.com/1morr/Notelet`,repo 真的公開之後再核對。
  - `author.url`:目前是 `https://github.com/1morr`,確認帳號頁面存在。
  - `id` 是 `1morr.notelet`,對應投稿 repo 裡的資料夾 `extensions/1morr/notelet/`,
    兩邊必須一致(CI 會驗)。
...
- `categories` 最多 3 個,只能從固定清單挑;Notelet 用 `productivity`。
- `tags` 最多 5 個、每個 ≤30 字。
...
3. 建 `extensions/1morr/notelet/`,放入 `extension.json` 與 `icon.png`(可加 `screenshots/`)。
...
為基準算的 —— 檔案放進 `microsoft/CmdPal-Extensions` 的 `extensions/<author>/notelet/`
