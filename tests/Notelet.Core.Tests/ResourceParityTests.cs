...
/// 三份 .resx 的一致性。
///
/// **為什麼這個測試在 Core 的測試專案裡** —— 那三個檔案屬於 <c>src/Notelet</c>(UI 層),
...
public class ResourceParityTests
{
    ...
    {
        // tests/Notelet.Core.Tests/ResourceParityTests.cs -> repo 根目錄
        var repositoryRoot = Path.GetFullPath(Path.Combine(Path.GetDirectoryName(callerPath)!, "..", ".."));

        return Path.Combine(repositoryRoot, "src", "Notelet", "Properties");
    }
}
