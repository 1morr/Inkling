...
Inkling 把記筆記這件事加進 Command Palette:幾秒鐘記下一個想法，之後就在同一個地方瀏覽、搜尋、編輯，不必再開其他程式。筆記是你指定資料夾裡的純 Markdown 檔案，不是鎖在某個程式裡的紀錄:任何編輯器都打得開、雲端硬碟自然會同步，就算 Inkling 消失了也一個字都不會少。所以同步、手機存取，以及比較繁重的編輯，交給你已經在用的雲端硬碟和編輯器。
