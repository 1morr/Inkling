...
| 新增(完整) | `Inkling：新增筆記` 開表單,可寫多行內文;在清單頁按 `Ctrl+N` 也能直接開。存檔會**跳「已新增：<標題>」並收起面板**,跟快速記下一樣 —— 編輯既有筆記剛好相反,存完留在原地([為什麼](docs/design-notes.md#edit-form)) |
...
| 刪除 | 清單頁 `Ctrl+D`,確認後**移到資源回收筒**,底部會講出刪掉的是哪一則。**選取會落在下一則上**,連著刪好幾則不會被丟回最上面([為什麼這件事不是免費的](docs/design-notes.md#selection-survives-rebuild))。網路磁碟、或沒有資源回收筒的裝置上,Windows 會直接永久刪除 ——**刪除筆記**那一頁的詳細窗格會講明白 |
...
編輯表單的**存檔是 `Tab` 到「儲存」再按 `Enter`**,存完**這一頁會留著**,按 `Esc` 回上一頁。
焦點在單行的標題欄時按 `Enter` 刻意什麼都不做 —— 那一下以前會直接跳去外部編輯器並收掉面板,
卡片上打過的字跟著消失。
...
- **`id` 才是身分,檔名只是給人看的。** 改標題不會重新命名檔案 —— 在雲端同步資料夾裡
  頻繁 rename 是產生重複檔與衝突檔的頭號原因。清單上某一列**動到哪個檔案**看的是它的路徑,
  不是 `id`,所以雲端的衝突副本(同一個 `id`、不同檔案)兩份各自獨立編輯得動。
- **讀不懂的日期不去動它。** `created` / `updated` 讀寫都走 ISO 8601
  (`2026-08-10T14:30:52+08:00`)。其他寫法 —— `2024-01-05 (approx)`,或是有歧義的
  `05/01/2024` —— 原字串留著,不猜、也不改寫。
...
- **沒有 front matter 的 `.md` 照樣會出現在清單裡**,標題取內文的第一行有效文字、
  ...
  ([為什麼](docs/design-notes.md#delete-page))。「Inkling 建立的」判準是 `id` 長成
  Inkling 產的那個形狀 —— 你自己的 `id:`(Zettelkasten、Hugo、Obsidian)會讓那個檔案
  留在「不是我建立的」那一邊,而且 Inkling 不會去覆蓋它。
- **檔案必須是 UTF-8。** 其他編碼(沒有 BOM 的 Big5、GBK、Latin-1)會整個跳過,不會被讀成
  亂碼,清單最後會講有幾個檔案讀不出來。有 BOM 的檔案 —— UTF-8、UTF-16 LE/BE —— 讀得到。
  這是刻意的:硬讀的話每個非 ASCII 位元組都會變成 ,而在 Inkling 裡編輯一次就把那串
  永久寫回檔案。
- 會掃子資料夾,新筆記一律寫在根目錄。
...
| 設定 | 預設 | 說明 |
|---|---|---|
| 筆記資料夾 | `%OneDrive%\Inkling`;沒有 OneDrive 時是 `Documents\Inkling` | 只接受**完整路徑**(相對路徑會整筆拒絕);指向還不存在的資料夾會當場提示,第一次存檔時建立。旁邊的「瀏覽…」開系統的選資料夾對話框,選好就直接存 |
| 快速記下的分隔符 | `;;` | 前面是標題、後面是內文。長度不限,半形全形算同一個,清空就回到 `;;`。改完當下開著的快速記下頁就會跟上,不必 Reload |
| 記下後先看一眼 | 開啟 | Enter 記下並停在筆記上,再按一次才收起;關掉就是記完直接收起 |

存成功會跳一則 toast 並回到主搜尋框;被拒絕的(相對路徑、寫檔失敗)則留在表單上、
打過的值還在,改完再送一次就好。
...
同一則筆記時,OneDrive 會產生 `檔名-電腦名.md` 這種副本。兩份都會出現在清單裡,而且因為
副本帶著**跟原檔一樣的 `id`**,Inkling 會把那兩列標上**衝突副本**,讓你知道發生了什麼。
編輯或刪除某一列動到的是**那一列自己的檔案**,兩份各自獨立。Inkling 不會替你合併:
在詳細窗格裡比一下兩列的內容,留下要的那份,把另一份刪掉。
